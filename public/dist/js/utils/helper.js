(() => {
    ...
    var S = $((f) => {
        ...
        var R = /^#([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})?$/i,
            ...
            d = /var\(--(?:[^ )]*?)(?:,(?:[^ )]*?|var\(--[^ )]*?\)))?\)/,
            x = new RegExp(
                `^(rgba?)\\(\\s*(${u.source}|${d.source})(?:${c.source}(${u.source}|${d.source}))?(?:${c.source}(${u.source}|${d.source}))?(?:${M.source}(${u.source}|${d.source}))?\\s*\\)$`
            ),
            N = new RegExp(
                `^(hsla?)\\(\\s*((?:${u.source})(?:deg|rad|grad|turn)?|${d.source})(?:${c.source}(${u.source}|${d.source}))?(?:${c.source}(${u.source}|${d.source}))?(?:${M.source}(${u.source}|${d.source}))?\\s*\\)$`
            );
        function V(e, { loose: r = !1 } = {}) {
            var t, o;
            ...
            return a.length === 2 && a[0].startsWith("var(")
                ? { mode: i[1], color: [a[0]], alpha: a[1] }
                : (!r && a.length !== 3) ||
                  (a.length < 3 && !a.some((n) => /^var\(.*?\)$/.test(n)))
                ? null
                : {
                      mode: i[1],
                      color: a,
        ...
        function B({ mode: e, color: r, alpha: t }) {
            let o = t !== void 0;
    ...
    var _ = O(S()),
        v = {
            ...
            randomNumbers(e, r, t) {
                let o = [0];
                for (let l = 1; l < t; l++)
                    o.push(Math.ceil(Math.random() * (e - r) + r));
                return o;
            },
            replaceAll(e, r, t) {
            ...
            watchCssVariables(e, r, t) {
                ...
                l.observe(o, { attributes: !0 });
                let s = r.reduce(
                    (i, a) => (
                        (i[a] = getComputedStyle(o)
                            .getPropertyValue(`--${a}`)
                            .trim()),
                        i
                    ),
