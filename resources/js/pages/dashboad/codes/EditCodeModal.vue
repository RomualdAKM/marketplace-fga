...
const form = reactive({
    ...
    percentage: props.code.percentage,
})
...
const editCode = async () => {
    ...
    formData.append("code", props.code.code);
    formData.append("percentage", props.code.percentage);

    await axios.post("/api/update_code/" + props.code.id, formData).then((response) => {
...
<template>
    <!-- BEGIN: Modal Content -->
    <div data-tw-backdrop="" aria-hidden="true" tabindex="-1" id="edit-code-modal-preview" class="modal group bg-gradient-to-b from-theme-1/50 via-theme-2/50 to-black/50 transition-[visibility,opacity] w-screen h-screen fixed left-0 top-0 [&:not(.show)]:duration-[0s,0.2s] [&:not(.show)]:delay-[0.2s,0s] [&:not(.show)]:invisible [&:not(.show)]:opacity-0 [&.show]:visible [&.show]:opacity-100 [&.show]:duration-[0s,0.4s]">
